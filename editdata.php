	...
	<div class="row content d-flex justify-content-center">
        <div class="col-lg-6">
            ...
            <p style="text-align: center;">
              Edit form berikut dengan baik dan benar :
            </p>
        </div>
        <form method="post" action ="successe.php">
        <input type="hidden" name="id" value="<?php echo $idcst['idcst']?>" />
            <br>
                <label for="nama">Nama</label>
                <input required type="text" value="<?php echo $idcst['namacst']?>" class="form-control" id="nama" name="nama">
            <br>
                <label for="nim">Nomor Hp</label>
                <input required type="number" require class="form-control" value="<?php echo $idcst['nohp']?>" id="hp" name="hp">
            <br>
                <label for="prodi">Tenggat Waktu</label>
                <input required type="text" class="form-control" value="<?php echo $idcst['deadline']?>" id="waktu" name="waktu">
            <br>
                <label for="email" class="form-label">Email address</label>
                <input required type="email" class="form-control" value="<?php echo $idcst['emcst']?>" id="email" name="email" aria-describedby="emailHelp" >
            ...
            <?php while($r = $results->fetch(PDO::FETCH_ASSOC)){?>
                  <option value="<?php echo $r['id_serv'] ?>" <?php if($r['id_serv'] == 
                    $idcst['id_serv']) echo 'selected'?>>
                    <?php echo $r['nama_serv']; ?>
                </option>
            <?php }?>
            </select>
            <br>
